...
def make_sample_assigments(n=1000, num_arms=3, seed = 42):
    np.random.seed(seed)
    ...
    assignments = []
    for i in range(1, n):
        arm_id = np.random.choice(arm_ids, size = 1, replace = True)
        arm_name = arms[arm_id]
        assignments.append(
            Assignment(participant_id=i, arm_id = arm_id, 
                       arm_name = arms[arm_id], strata = []))
    return(assignments)

def make_sample_outcomes(assignments, seed = 42):
    outcomes = [ParticipantOutcome(
        participant_id = [assignment.participant_id for assignment in assignments],
        metric_value = [np.random.choice([0, 1]) for _ in assignments]
    )]
    return(outcomes)

@pytest.fixture
def test_assignments():
    return pd.DataFrame(make_sample_assignments())

@pytest.fixture
def test_outcomes():
    return pd.DataFrame(make_sample_outcomes())
