...
def calculate_mde_with_desired_n(
    ...
    """
    Calculate MDE for individual or cluster randomization.

    Returns tuple of (target_value, pct_change)
    """
    if desired_n <= 0:
    ...
    if icc is not None and avg_cluster_size is not None:
        from xngin.stats.cluster_power import calculate_mde_cluster  # noqa: PLC0415

        analysis = calculate_mde_cluster(
        ...
        analysis = target_possible, pct_change_possible
    return analysis
...
def _analyze_power_sample_size_mode(
    ...
    If icc and avg_cluster_size are provided, performs cluster analysis.
    Otherwise performs individual randomization analysis.
    """
    ...
    if icc is not None and avg_cluster_size is not None:
        ...
        analysis = analyze_metric_power_cluster(
            metric=metric,
            n_arms=n_arms,
            icc=icc,
            avg_cluster_size=avg_cluster_size,
