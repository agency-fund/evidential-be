...
logging.getLogger("sqlalchemy.engine.Engine.xngin_dwh").setLevel(logging.INFO if flags.LOG_SQL else logging.WARN)

# Conditionally enable the logging of SQL generated by SQLAlchemy for the xngin app database.
logging.getLogger("sqlalchemy.engine.Engine.xngin_app").setLevel(logging.INFO if flags.LOG_SQL_APP_DB else logging.WARN)
...
def setup():
    ...
    loggers = sorted([logging.getLogger(name).name for name in logging.root.manager.loggerDict])
    for name in loggers:
