...
async def test_initial_user_setup_matches_testing_dwh(xngin_session):
    ...
    assert len(first_user.organizations[0].datasources) == 2

    # First datasource is the testing dwh.
    ...
    # Second datasource is the autogenerated NoDwh source.
    ds = first_user.organizations[0].datasources[1]
    ds_config = ds.get_config()
    assert isinstance(ds_config.dwh, NoDwh)
...
def test_datasource_lifecycle(ppost, pget, ppatch):
    ...
    # First datasource is the testing dwh
    assert list_ds_response.items[0].driver == "none"
    # Second datasource is the one we created
    assert list_ds_response.items[1].id == datasource_id
    assert list_ds_response.items[1].name == "test remote ds"
    assert list_ds_response.items[1].organization_id == org_id
    assert list_ds_response.items[1].driver == "postgresql+psycopg"
    ...
    assert response.status_code == 200, response.content
    assert (
        ListDatasourcesResponse.model_validate(response.json()).items[1].name
        == "updated name"
    )
