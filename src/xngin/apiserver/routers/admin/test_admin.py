...
async def test_datasources_hide_credentials(
    ...
    before_revision = (
        await xngin_session.get(tables.Datasource, datasource_id)
    ).get_config()
    match dsn:
        case PostgresDsn() | RedshiftDsn():
        ...
        case BqDsn():
            ...
            revised_bq.credentials = GcpServiceAccount(
                content=json.dumps(revised_service_account)
            )
            update_request = UpdateDatasourceRequest(dsn=revised_bq)
    response = ppatch(
        f"/v1/m/datasources/{datasource_id}", content=update_request.model_dump_json()
    )
    ...
    match dsn:
        ...
        case BqDsn():
            ...
            assert (
                "newprivatekey"
                in base64.standard_b64decode(
                    after_revision.dwh.credentials.content_base64
                ).decode()
            )
    ...
    match dsn:
        ...
        case BqDsn():
            ...
            assert (
                after_second_revision.dwh.credentials == after_revision.dwh.credentials
            )
            assert (
                "newprivatekey"
                in base64.standard_b64decode(
        ...
        content=UpdateDatasourceRequest(
            dsn=BqDsn(
                ...
                credentials=GcpServiceAccount(
                    content=SAMPLE_GCLOUD_SERVICE_ACCOUNT_JSON
                ),
            )
        ).model_dump_json(),
    )
