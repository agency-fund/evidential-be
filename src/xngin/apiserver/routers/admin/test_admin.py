...
def find_ds_with_name(
    datasources: list[tables.Datasource | DatasourceSummary], name: str
...
def test_datasource_lifecycle(ppost, pget, ppatch):
    ...
    no_dwh = find_ds_with_name(list_ds_response.items, DEFAULT_NO_DWH_SOURCE_NAME)
    assert no_dwh.driver == "none"
    # Ensure we have a test dwh source
    test_dwh = find_ds_with_name(list_ds_response.items, new_ds_name)
