...
def humane_operation_id(route: APIRoute) -> str:
    """Derives the OpenAPI operationId from the Python endpoint function name.

    Wired up as the app's ``generate_unique_id_function`` so operationIds read like
    ``list_experiments`` instead of FastAPI's autogenerated
    ``list_experiments_v1_experiments_get``, keeping generated client method names humane.
    """
    return route.name
...
def custom_openapi(app: FastAPI):
    ...
    # Two routes resolving to the same operationId would silently collide in generated clients, so we
    # guard against it.
    seen: set[str] = set()
