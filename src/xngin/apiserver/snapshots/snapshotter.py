...
async def make_first_snapshot(experiment_id: str, snapshot_id: str):
    ...
    async with database.async_session() as session, session.begin():
        ...
        await query_for_snapshot_data(session, datasource, experiment)
        ...
        async with database.async_session() as session, session.begin():
            ...
            try:
                async with asyncio.timeout(snapshot_timeout):
                    snapshot.data = await query_for_snapshot_data(session, datasource, experiment)
...
async def query_for_snapshot_data(
