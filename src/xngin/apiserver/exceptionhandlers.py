...
# These mirror the types generated by FastAPI for 422s. Use these when building 422 responses.
class XValidationError(BaseModel):
    loc: Sequence[str | int]
    msg: str
    type: str


class XHTTPValidationError(BaseModel):
    detail: Sequence[XValidationError]


def as_fastapi_http_validation_error(msg: str, type_: str = "late"):
    return XHTTPValidationError(detail=[XValidationError(loc=[0], type=type_, msg=msg)]).model_dump()
