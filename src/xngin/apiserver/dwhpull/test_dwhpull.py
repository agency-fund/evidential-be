...
def make_mab_dwh_experiment(xngin_session, datasource, **kwargs) -> tables.Experiment:
    return insert_experiment_and_arms(
        xngin_session,
        datasource,
...
def read_outcomes(xngin_session, experiment_id: str) -> dict[str, float | None]:
...
def test_pull_one_experiment_boolean_target(xngin_session, testing_datasource):
    """A pull reads target values for outcome-less draws, applies them, and reports counts."""
    experiment = make_mab_dwh_experiment(xngin_session, testing_datasource.ds, target_field_name="is_onboarded")
    ...
    assert read_outcomes(xngin_session, experiment.id) == {"1": 0.0, "2": 1.0, "99999999999": None}
...
def test_pull_one_experiment_with_no_outcomeless_draws_skips_the_warehouse(xngin_session, testing_datasource):
    """The common case: nothing to read, so the run does not touch the data warehouse at all."""
    experiment = make_mab_dwh_experiment(xngin_session, testing_datasource.ds)
...
def test_pull_one_experiment_numeric_target_accepts_any_float(xngin_session, testing_datasource):
    ...
    outcomes = read_outcomes(xngin_session, experiment.id)
    assert outcomes["1"] == 29912.0


def test_pull_one_experiment_null_target_value_stays_pending(xngin_session, testing_datasource):
    """A row whose target column is NULL is left pending, to be re-read on a later run."""
    experiment = make_mab_dwh_experiment(
        xngin_session,
        testing_datasource.ds,
...
def test_pull_one_experiment_invalid_value_skipped_and_left_null(xngin_session, testing_datasource):
    ...
    assert read_outcomes(xngin_session, experiment.id) == {"1": None}
    for arm in experiment.arms:
        xngin_session.refresh(arm)
...
def test_pull_one_experiment_continues_after_a_rejected_value(xngin_session, testing_datasource):
...
def test_pull_one_experiment_leaves_resolved_draws_alone(xngin_session, testing_datasource):
    ...
    assert (report.ingested, report.invalid, report.pending) == (1, 0, 0)
    assert read_outcomes(xngin_session, experiment.id) == {"1": 0.0, "2": 1.0}


def test_pull_all_experiments_abandons_an_experiment_that_exceeds_its_timeout(
    ...
    assert read_outcomes(xngin_session, experiment.id) == {"2": None}


def test_a_timed_out_pull_releases_its_draw_locks(xngin_session, testing_datasource, mocker):
    ...
    report = pull_one_experiment(experiment.id)
    assert (report.ingested, report.invalid, report.pending) == (1, 0, 0)
    assert read_outcomes(xngin_session, experiment.id) == {"2": 1.0}


def test_pull_one_experiment_applies_all_outcomes_or_none(xngin_session, testing_datasource, mocker):
    ...
    assert len(seen) == 2
    assert read_outcomes(xngin_session, experiment.id) == {"1": None, "2": None}


def test_pull_one_experiment_rejects_non_dwh_experiment(xngin_session, testing_datasource):
    experiment = insert_experiment_and_arms(
        xngin_session,
        testing_datasource.ds,
...
def test_select_experiments_to_pull_skips_other_types_and_states(xngin_session, testing_datasource):
    due = make_mab_dwh_experiment(xngin_session, testing_datasource.ds)
    make_mab_dwh_experiment(xngin_session, testing_datasource.ds, state=ExperimentState.ASSIGNED)
    make_mab_dwh_experiment(
        xngin_session,
        testing_datasource.ds,
...
def test_pull_all_experiments_isolates_a_failing_experiment(xngin_session, testing_datasource):
    """One experiment that cannot be read must not stop the others in the same run."""
    broken = make_mab_dwh_experiment(xngin_session, testing_datasource.ds, target_field_name="is_onboarded")
    broken.datasource_table = "no_such_table"
    healthy = make_mab_dwh_experiment(xngin_session, testing_datasource.ds, target_field_name="is_onboarded")
    xngin_session.commit()
    ...
    assert read_outcomes(xngin_session, broken.id) == {"1": None}
    assert read_outcomes(xngin_session, healthy.id) == {"2": 1.0}


def test_pull_all_experiments_covers_every_due_experiment(xngin_session, testing_datasource):
    first = make_mab_dwh_experiment(xngin_session, testing_datasource.ds, target_field_name="is_onboarded")
    second = make_mab_dwh_experiment(xngin_session, testing_datasource.ds, target_field_name="is_onboarded")
    ...
    assert read_outcomes(xngin_session, first.id) == {"1": 0.0}
    assert read_outcomes(xngin_session, second.id) == {"2": 1.0}
